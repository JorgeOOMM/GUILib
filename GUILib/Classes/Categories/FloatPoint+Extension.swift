...
public extension CGFloat {
    ...
    private static func solveQuadratic(_ cuadraticA: CGFloat, _ cuadraticB: CGFloat, _ cuadraticC: CGFloat) -> CGFloat {
        var result = (-cuadraticB + sqrt(cuadraticB.squared - 4 * cuadraticA * cuadraticC)) / (2 * cuadraticA);
        guard !result.isInRangeOrEqual(0, 1) else {
            return result
        }
        
        result = (-cuadraticB - sqrt(cuadraticB.squared - 4 * cuadraticA * cuadraticC)) / (2 * cuadraticA);
        guard !result.isInRangeOrEqual(0, 1) else {
            return result
        }
