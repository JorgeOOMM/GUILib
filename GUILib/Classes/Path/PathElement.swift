...
extension Path {
    public enum Element {
        ...
        func percentagesWhereYIs(y: Double,
                                 ifStartedFrom from: CGPoint?,
                                 subpathStartedFrom subpathFrom: CGPoint?,
                                 precalculatedLength: Double) -> [Double] {
            switch self {
            case .moveToPoint:
                return []
            case let .addLineToPoint(point):
            ...
            case let .addQuadCurveToPoint(destination, control):
                if let haveFrom = from {
                    ...
                    let roots: Array<Double> = Double.solveQuadraticEquation(solveA: a,
                                                                             ...
                                                                             solveC: c)
                    var result: [Double] = []
                    for i in stride(from: 0, to: roots.count, by: 1) {
                        let root = roots[i]
                        if (root >= 0) && (root <= 1) {
                            result.append(root)
                        }
            ...
            case .closeSubpathWithLine:
                if let haveFrom = from, let haveTo = subpathFrom {
        ...
        func pointForPercentage(pathPercent: Double,
                                ifStartedFrom from: CGPoint?,
                                subpathStartedFrom subpathFrom: CGPoint?) -> CGPoint? {
            switch self {
            case .moveToPoint:
                return nil
            case let .addLineToPoint(point):
            ...
            case let .addCurveToPoint(destination, control1, control2):
                if let haveFrom = from {
                    let point = Path.pointOfCubic(t: pathPercent, 
                                                  from: haveFrom,
                                                  to: destination,
                                                  c1: control1,
                                                  c2: control2)
